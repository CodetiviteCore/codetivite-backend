...
exports.updateUser = async (request, response, next) => {
  ...
  const reqBodySchema = yup.object({
    ...
    careerPath: yup.string().oneOf(careers).required(),
    aboutMe: yup.string().required(),
  }); 
  try {
    await reqBodySchema.validate(request.body, { abortEarly: false });
  } catch (error) {
    if (error.name === "ValidationError") {
      ...
      return response.status(BAD_REQUEST).send({ message: "Validation failed." });
    }
  }
  try {
