...
exports.getUser = async (request, response, next) => {
  ...
  try {
    ...
    if (!user) {
      return response.status(NOT_FOUND).send({ message: 'Invalid user Id' });
    }
  } catch (error) {
    next(error);
  }
...
exports.updateUser = async (request, response, next) => {
  ...
  if (!request.body || Object.keys(request.body).length === 0) {
    return response.status(BAD_REQUEST).send({ message: "Request body is empty." });
  }

  const requiredFields = ["firstName", "lastName", "careerPath"];
  const missingFields = requiredFields.filter(field => !(field in request.body));
  if (missingFields.length > 0) {
    return response.status(BAD_REQUEST).send({ message: `Missing required fields: ${missingFields.join(", ")}` });
  }
  
  const reqBodySchema = yup.object({
    firstName: yup.string(),
    lastName: yup.string(),
    careerPath: yup.string().oneOf(careers),
  });  
  const {error, value} = reqBodySchema.validate(request.body, {abortEarly: false});
  if(error) {
    let errorMessage = [];
  ...
  try {
    const updatedUser = await userModel.findOneAndUpdate({ _id }, { $set: value }, { new: true });
    response.status(OK).send({ message: "success", updatedUser });
  } catch (error) {
    next(error);
  }
};
