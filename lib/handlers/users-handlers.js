...
exports.updateUser = async (request, response, next) => {
  try {
    if (!request.user) {
      return response.status(BAD_REQUEST).send({ message: 'User not authenticated' });
    }
  } catch (error) {
    next(error);
  }
  ...
  try {
    const updatedUser = await userModel.findOneAndUpdate({ _id }, { $set: request.body }, { new: true });
