...
exports.loginWithMagicLink = async (request, response, next) => {
  ...
  const whitelistEmail = await whitelistModel.findById(email);
  ...
  try {
    ...
    if (user && user.lastName) {
      ...
      // To update existing user's login activity
      await calculateLoginActivity(user)
  ...
  try {
    if (!user) {
      ...
      user = new userModel({
        ...
        dailyLoginActivity: [await calculateLoginActivity(user)], //To add login activity for new user
      });
...
// To calculate login activity
const calculateLoginActivity = async (user) => {
  const today = new Date();
  today.setHours(0, 0, 0, 0)

  const dailyActivityEntry = user.dailyLoginActivity.find(entry => entry.date.getTime() === today);

  if(dailyActivityEntry) {
    dailyActivityEntry.totalActivity += ONE
  } else {
    user.dailyLoginActivity.push({
      date: today,
      totalActivity: ONE
    });
  }
