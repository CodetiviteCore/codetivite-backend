...
exports.getUserRoadMap = async (request, response) => {
  ...
  const levelAndDescriptions = Object.keys(levels).filter(key => levels[key].length > 0).map(level => {
    return {
      roadmap: level,
      roadmapId: level,
  ...
  resource = resource.map(x => {
    return {
      ...
      isCompleted: usersCompletedProjectIds.includes(x?.projectId)
    };
  })
...
exports.getRoadMap = async (request, response, next) => {
  const { careerPath, level } = request.query;
  ...
  try {
    ...
    if (resource && level) {
      resource = resource?.levels[0][level];
      return response.status(OK).send({ message: "Success", resource })
    }
